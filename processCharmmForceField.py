...
for inputfile in sys.argv[1:]:
    for line in open(inputfile):
        ...
        if line == 'read rtf card':
            ...
            elif (fields[0] == 'BOND') or (fields[0] == 'DOUBLE'):
...
print(' </CMAPTorsionForce>')
print(' <NonbondedForce coulomb14scale="0.5" lj14scale="0.5">') # NEED TO FIX
for type in atomTypes:
...
customNonbondedForce = mm.CustomNonbondedForce('4*epsilon*((sigma/r)^12-(sigma/r)^6); sigma=sqrt(sigma1*sigma2); epsilon=sqrt(epsilon1*epsilon2)')
...
customNonbondedForce.setNonbondedMethod(nonbondedForce.getNonbondedMethod())
customNonbondedForce.setUseSwitchingFunction(nonbondedForce.getUseSwitchingFunction())
customNonbondedForce.setCutoffDistance(nonbondedForce.getCutoffDistance())
customNonbondedForce.setSwitchingDistance(nonbondedForce.getSwitchingDistance())
customNonbondedForce.setUseLongRangeCorrection(nonbondedForce.getUseDispersionCorrection())
...
for i in range(nonbondedForce.getNumParticles()):
    ...
    customNonbondedForce.addParticle([sig,eps])
